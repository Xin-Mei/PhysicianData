...
(function (root) {
  ...
  function renderReport(data, cfg) {
    ...
    const clinics = clinicsAll.filter(c => !cfg.clinics || !cfg.clinics.length || cfg.clinics.indexOf(c) >= 0);  // ['__none__'] = 全部取消
    ...
    groups.forEach(gp => h += '<th colspan="' + gp.cols.length + '" class="g">' + groupHtml(gp.g) + '</th>');
  ...
  .rpt th.g{font-size:12px;line-height:1.2}
  .rpt th.g .gsub{font-size:9.5px;font-weight:500;color:#555}
  ...
  .rpt td.r.top b{color:inherit;font-weight:800}
  ...
  function prettyTable(P, cfg, pd, topC, lim, data) {
    ...
    groups.forEach(g => h += `<th colspan="${g.cols.length}" class="g" style="border-bottom:3px solid ${g.color}">${groupHtml(g.g)}</th>`);
      ...
      rows.forEach((r, ri) => {
        ...
        if (csCols.length && ri === 0) {
          ...
          csCols.forEach(x => {
            ...
            if (x[0] === 'rank') v = P.csRank[cl] ? rankBadge(P.csRank[cl], P.csRank[cl] <= 3) : '';
            h += `<td class="cs" rowspan="${rows.length}">${v}</td>`;
          });
        }
        h += '</tr>';
  ...
  .pretty .p-tb th.g{font-size:19px;line-height:1.25}
  ...
  .pretty .p-tb td.c-doc{text-align:center;padding:10px 8px;width:130px;font-size:23px}
  ...
  .pretty .cc-t .l{text-align:center;font-size:20px}
  ...
  function renderVisits(data, opt) {
    ...
    const cur = (data.clinicSum || []).filter(c => numOrNull(c.total) !== null && +c.total > 0);
    const prev = {}; (data.prevClinicSum || []).forEach(c => { if (numOrNull(c.total) !== null) prev[c.clinic] = +c.total; });
    const list = cur.map(c => ({ clinic: c.clinic, total: +c.total })).sort((a, b) => b.total - a.total);
    const grand = list.reduce((a, c) => a + c.total, 0);
    const pgrand = list.every(c => prev[c.clinic] !== undefined) && list.length ? list.reduce((a, c) => a + prev[c.clinic], 0) : null;
    const max = list.length ? list[0].total : 1;
    const avg = list.length ? Math.round(grand / list.length) : 0;
    const d = new Date(); const today = d.getFullYear() + ' / ' + String(d.getMonth() + 1).padStart(2, '0') + ' / ' + String(d.getDate()).padStart(2, '0');
    const diff = (a, b) => { if (b === undefined || b === null) return ''; const v = a - b; return `<span class="vz-d ${v > 0 ? 'up' : v < 0 ? 'dn' : ''}">較上月 ${v > 0 ? '▲' : v < 0 ? '▼' : '－'} ${Math.abs(v).toLocaleString()}</span>`; };
    let h = `<div class="vz"><div class="vz-top"></div><div class="vz-in">
      <div class="vz-hd"><div class="vz-brand"><img src="${LOGO}" alt="">金鶯診所<span>ELITE CLINIC</span></div><div class="vz-crumb">醫師回診率追蹤 ｜ 全院區看診總人次</div></div>
      <div class="vz-row">
        <div><div class="vz-eyebrow">MONTHLY REPORT — CLINIC VISITS</div><div class="vz-title">全院區看診總人次</div>
          <div class="vz-sub">資料期間 ${y - 1911} 年 ${m} 月<i>｜</i>依總人次排序</div></div>
        <div class="vz-sum"><div><div class="l">全院總計 · ${list.length} 院區</div><div class="v">${grand.toLocaleString()}<small>人次</small></div>${pgrand !== null ? `<div class="s">${diff(grand, pgrand)}</div>` : ''}</div>
          <div class="sep"></div><div><div class="l">院區平均</div><div class="v">${avg.toLocaleString()}<small>人次／院區</small></div></div></div>
      </div>
      <div class="vz-grid">${list.map((c, i) => `<div class="vz-card${i === 0 ? ' first' : ''}">
          <div class="vz-n"><b>${String(i + 1).padStart(2, '0')}</b>${esc(c.clinic)}</div>
          <div class="vz-v"><span>${c.total.toLocaleString()}</span><em>佔全院 ${(c.total / grand * 100).toFixed(1)}%</em></div>
          <div class="vz-bar"><i style="width:${(c.total / max * 100).toFixed(1)}%"></i></div>
  ...
  .vz-brand span{margin-left:14px;letter-spacing:1px}
  .vz-crumb{font-size:22px;color:#6f6f6a}
  .vz-row{display:flex;justify-content:space-between;align-items:flex-end;margin:40px 0 34px}
  .vz-eyebrow{font-size:26px;font-weight:700;color:#b08a47;letter-spacing:1px}
  .vz-title{font-size:72px;font-weight:800;color:#1d5c4a;margin-top:10px;letter-spacing:2px;line-height:1.1}
  .vz-sub{font-size:25px;color:#6f6f6a;margin-top:18px}
  .vz-sub i{font-style:normal;margin:0 24px;color:#bdbab2}
  .vz-sum{display:flex;align-items:center;background:#1d5c4a;border-radius:24px;padding:26px 50px;gap:50px;color:#fff;min-width:800px;justify-content:space-around}
  .vz-sum .l{font-size:21px;color:#d9b777;font-weight:700;text-align:center}
  .vz-sum .v{font-size:50px;font-weight:800;text-align:center;margin-top:6px;line-height:1.1}
  .vz-sum .v small{font-size:22px;color:#d9b777;font-weight:500;margin-left:10px}
  .vz-sum .s{text-align:center;margin-top:6px;font-size:17px}
  .vz-sum .s .vz-d{color:#cfe3dc}
  .vz-sum .sep{width:1px;align-self:stretch;background:#3f7a68}
  .vz-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:34px 36px}
  .vz-card{background:#faf5ea;border:3px solid #eee3cf;border-radius:26px;padding:24px 30px 20px}
  .vz-card.first{background:#f4e8cd;border-color:#c9a466}
  .vz-n{font-size:32px;font-weight:800;color:#1d5c4a;display:flex;align-items:baseline;gap:16px}
  .vz-n b{color:#b08a47;font-size:29px}
  .vz-v{display:flex;justify-content:space-between;align-items:baseline;margin-top:10px}
  .vz-v span{font-size:48px;font-weight:800;color:#222;letter-spacing:1px}
  .vz-card.first .vz-v span{color:#b08a47}
  .vz-v em{font-style:normal;font-size:19px;color:#77756f}
  .vz-bar{height:9px;background:#efe7d6;border-radius:5px;margin-top:14px;overflow:hidden}
  .vz-bar i{display:block;height:100%;background:#1d5c4a;border-radius:5px}
  .vz-card.first .vz-bar i{background:#b08a47}
  .vz-cmp{margin-top:10px;font-size:17px;min-height:24px}
  .vz-d{color:#8d8b85}
  .vz-d.up{color:#3e8a64}.vz-d.dn{color:#c0735f}
  .vz-note{display:flex;align-items:center;gap:30px;background:#1d5c4a;border-radius:18px;padding:22px 36px;margin-top:40px;color:#fff;font-size:25px}
  .vz-note .tag{display:inline-block;background:#c7a36a;color:#fff;font-weight:700;font-size:24px;line-height:34px;border-radius:10px;padding:6px 40px;white-space:nowrap}
  .vz-ft{display:flex;justify-content:space-between;margin-top:30px;font-size:20px;color:#8d8b85}
  `;
  ...
  root.Engine = { DEFAULT_CONFIG, LOGO, renderPretty, PRETTY_CSS, renderVisits, VISITS_CSS, normCode, num, numOrNull, rocDay, isoDay, dayIso, dayRoc, monthLabel, prevMonth, monthRange, r2, r4,
